...
function CompleteRegistrationContent() {
  ...
  const [userInfo, setUserInfo] = useState<{ name: string; last_name?: string; email: string } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/auth/complete-registration?token=${token}`)
      .then(async (res) => {
        if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        setError(err.error || "Error al completar el registro");
      }
    } catch {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-[var(--color-tropical-teal)] to-[var(--color-electric-sapphire)] p-4">
      <Card className="w-full max-w-md shadow-2xl bg-[var(--color-carbon-black)] border border-divider/10">
        <CardHeader className="flex flex-col items-center pt-8 pb-4">
          ...
          <p className="text-sm text-[var(--color-lavender-mist)]/70 mt-2">Completa tu Registro</p>
        </CardHeader>
        ...
        <CardBody className="p-8">
          {isValidating ? (
            <div className="flex flex-col items-center gap-4 py-8">
              <Spinner size="lg" />
              <p className="text-sm text-[var(--color-lavender-mist)]/70">Validando invitación...</p>
            </div>
          ) : tokenError ? (
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <p className="text-[var(--color-lavender-mist)]/50 text-center text-xs">
                Contacta al administrador de tu organización para solicitar un nuevo enlace de invitación.
              </p>
            </div>
          ) : success ? (
            <div className="flex flex-col items-center gap-4 py-8">
              ...
              <p className="text-emerald-400 text-center font-medium">¡Registro completado!</p>
            ...
            <>
              <div className="mb-6 p-4 rounded-xl bg-white/5 border border-white/10">
                <p className="text-sm text-[var(--color-lavender-mist)]/70 mb-1">Registrando como</p>
                <p className="text-white font-semibold">{userInfo?.name} {userInfo?.last_name || ""}</p>

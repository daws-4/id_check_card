    ...
    <section>
              ...
              {loading ? (
                ...
                users.map((user, idx) => (
                  <tr key={user._id} className="hover:bg-[var(--color-lavender-mist)]/30 transition-colors group">
                    ...
                    <td className="py-4 px-6">
                      <span className={`px-3 py-1 text-xs font-medium rounded-full border ${user.role === 'superadmin' ? 'bg-[var(--color-tropical-teal)]/10 text-[var(--color-tropical-teal)] border-[var(--color-tropical-teal)]/20' : 'bg-[var(--color-maya-blue)]/10 text-[var(--color-maya-blue)] border-[var(--color-maya-blue)]/20'}`}>
                        {user.role === 'org_admin' ? 'Organización' : 'Sistema'}
                      </span>
                    </td>
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">{selectedUser ? "Editar Administrador" : "Crear Administrador"}</ModalHeader>
              <ModalBody>
                ...
                >
                  <SelectItem key="org_admin">Organización Local</SelectItem>
                  <SelectItem key="superadmin">Superadmin de la Plataforma</SelectItem>
                </Select>
                {role === "org_admin" && (
                  <Select 
                    label="Organización" 
                    placeholder="Selecciona la organización a administrar"
                    variant="bordered" 
                    selectedKeys={organizationId ? [organizationId] : []}
                    onChange={(e) => setOrganizationId(e.target.value)}
                  >
                    {organizations.map((org) => (
                      <SelectItem key={org._id} textValue={org.name}>{org.name}</SelectItem>
                    ))}
                  </Select>
                )}
                {!selectedUser && (
